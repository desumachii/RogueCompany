...
namespace ZeroGUI
{
	...
	void SetupCanvas(Canvas* _canvas)
	{
		canvas = _canvas;
	}
	...
	void TextLeft(char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		...
		outline = true;
		canvas->K2_DrawText(FString{ s2wc(name)}, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, false, true, false, Colors::Text_Outline);
	}
	void TextCenter(const char* name, FVector2D pos, FLinearColor color, bool outline)
	{
		outline = true;
		canvas->K2_DrawText( FString{ s2wc(name) }, pos, FVector2D{ 0.97f, 0.97f }, color, false, Colors::Text_Shadow, FVector2D{ pos.X + 1, pos.Y + 1 }, true, true, false, Colors::Text_Outline);
	}
